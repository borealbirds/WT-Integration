...
if (length(list.files(project_dir)) ==0) {
  pid <- WTpj_Tbl %>%
    filter(dataset_code =="BAM-BBS2025") %>%
...
data_flat <-do.call(rbind, outPtCount) # 2025
...
#write.csv(location_tbl, file= file.path(out_dir, "BAM-BBS2025_location.csv"), row.names = FALSE, na = "")
#location_out <- file.path(out_dir,"BAM-BBS2025_location.csv")
#drive_upload(media = location_out, path = as_id(dr_dataset_code), name = "BAM-BBS2025_location.csv", overwrite = TRUE) 
...
write.csv(survey_tbl, file= file.path(out_dir, "BAM-BBS2025_survey.csv"), na = "", row.names = FALSE)
survey_out <- file.path(out_dir, "BAM-BBS2025_survey.csv")
drive_upload(media = survey_out, path = as_id(dr_ls), name = "BAM-BBS2025_survey.csv", overwrite = TRUE) 

extended_tbl <- data_flat[!duplicated(data_flat[,Extended]), Extended] 
write.csv(extended_tbl, file.path(out_dir, "BAM-BBS2025_behavior.csv"), na = "", row.names = FALSE)
beh_out <- file.path(out_dir, "BAM-BBS2025_behavior.csv")
drive_upload(media = beh_out, path = as_id(dr_ls), name = "BAM-BBS2025_behavior.csv", overwrite = TRUE) 
